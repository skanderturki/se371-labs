...
const openBooksPage = (request, response) => {
  response.render('add-books', {title: 'Add Books', message: '', error: ''});
};

const openSearchPage = (request, response) => {
...
const deleteBook = (request, response) => {
  ...
  Book.findByIdAndDelete(id).then(() => {
    return response.json({successful: true, redirectTo: '/books/v1/'});
  }).catch(error => {
    console.log(error);
  });
}

module.exports = { openBooksPage, openSearchPage, search, add, deleteBook };